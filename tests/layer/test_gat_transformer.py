...
def test_gat_transformer_model():
    fx, fy, a = get_timeseries_graph_data()
    model_layer = GATTransformer(seq_len=fx.shape[-1], adj=a, gat_layer_sizes=[2], transformer_layers=1)
...
def test_gat_transformer_custom_aggregator():
    fx, fy, a = get_timeseries_graph_data()
    agg = lambda a, b: a - b
    layer = GATTransformer(seq_len=fx.shape[-1], adj=a, gat_layer_sizes=[2], transformer_layers=1, aggregator=agg)
    x_input, x_output = layer.in_out_tensors()
    model = Model(inputs=x_input, outputs=x_output)
    preds = model.predict(fx[:1])
